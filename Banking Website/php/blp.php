...
$query9 ="select current from customer where acno ='$acno';";
$res9=mysqli_query($connect,$query9);
$row9=$res9->fetch_assoc();
	$s9=$row9['current'];
if($s9>=$amount){
	$query2="select amount from business_loan where acno='$acno';";
	...
	if($k >= 0){
		$p=$s9-$amount;
		$query52="update customer set current=$p where acno='$acno';";
	 mysqli_query($connect,$query52);
	 $query3="update business_loan set amount='$k' where acno='$acno';";
	...
	$s1=$row['amount'];
	if($s1==0){
		 $query5="update customer set businessloan=FALSE where acno='$acno';";
	 mysqli_query($connect,$query5);
	$query51="delete from business_loan where acno='$acno';";
	 mysqli_query($connect,$query51);
	}
	echo "<body style='background-color:lightblue'>";
	echo "<br><br><br><br><br><br><br><br><br><br><h1 style='text-align:center;color:green;'><img src='sucess.png' width='40' height='40'><br>&nbsp;loan amount of $amount is sucessfully paid remaining balance is: $s1 that has to be paid</h1>";
	echo "</body>";	
	mail($mail,"BUSSINESS LOAN PAYMENT","Loan amount of $amount is sucessfully paid remaining balance is: $s1 that has to be paid");
	}
	else
	{
echo "<html><head></head>
...
if(confirm('no pending loan payments to pay/check the amount that has to be paid as house loan')){
	...
	echo "<br><br><br><br><br><br><br><br><br><br><h1 style='text-align:center;color:red;'><img src='warning.png' width='40' height='40'><br>&nbsp;no pending loan payments to pay/check the amount that has to be paid as business loan</h1>";
	echo "</body>";	
	}
}
else{
...
<body>
<script>
if(confirm('insufficient funds in your current account')){
	location.href='main.html';
}else{
}
</script>
